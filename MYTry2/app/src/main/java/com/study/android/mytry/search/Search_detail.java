...
public class Search_detail extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // i_time_term=intent.getStringExtra("challenge_start");
...
//            Log.d("테스트", " diffSeconds" + diffSeconds);
    ...
    public static long Dday(String mday) {
        ...
        long currentTime = cal.getTimeInMillis(); //현재시간

        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String CurrentTime = sdf.format(currentTime);
//            String strDate = sdf.format(cal);

        Log.d("테스트","현재시간"+currentTime);
        cal.set(year,month - 1 , day);
        long birthTime = cal.getTimeInMillis();
        int interval = (int)( birthTime - Integer.parseInt(CurrentTime) )/1000; //초
        Log.d("테스트",""+interval);


        int mDay    = interval / (60 * 60 * 24);
        int mHour   = (interval - mDay * 60 * 60 * 24) / (60 * 60);
        int mMinute = (interval - mDay * 60 * 60 * 24 - mHour * 3600) / 60;
        int mSecond = interval % 60;
